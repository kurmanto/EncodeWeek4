...
export class AppComponent {
  ...
  delegate(to: string) {
    if (to.toLowerCase() == 'self') to == this.wallet?.address;
  ...
  castVote(voteId: string, amount: string) {
    let param1 = Number(voteId);
    let param2 = Number(amount);
  ...
  postVote(address: string, amount: number) {
    try {
      this.http
        .post<any>('http://localhost:3000/post-vote', {
          address: address,
          amount: amount
        })
        .subscribe((ans) => {
  ...
  getVotes() {
    try {
      this.http
      .get<any>('http://localhost:3000/get-votes')
      .subscribe((ans) => {
        return ans;
      });
    } catch (error) {
